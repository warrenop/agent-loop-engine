...
const repo = path.resolve(here, "..", ".."); // Agent_Loop_Enhancement
let pass = 0, fail = 0;
...
ok("mcp.json 指向的引擎入口存在", fs.existsSync(cfg.args[0]));

const transport = new StdioClientTransport({
  command: cfg.command,
  args: cfg.args,
  env: { ...process.env, ...(cfg.env || {}) }, // 含 AGENT_LOOP_PROFILE=dev_warren_agent
...
ok("loop_start 载入 dev_warren_agent profile", txt(start).includes("Java/Spring") && txt(start).includes("mvn"));
...
await client.callTool({ name: "loop_record", arguments: { artifact: "context-map", mode: "overwrite",
  content: "## 需求摘要\n目标:集成测试需求,涉及品牌方小程序与门店后台。验收点:可按品牌方列可选门店并迁入商品。约束:不改 AMIS。" } });
...
// 5) INVESTIGATE:超预算预警(2c)。dev_warren_agent grep 预算=18
...
await client.callTool({ name: "loop_record", arguments: { artifact: "context-map", mode: "append",
  content: "## 调研发现\n" + "MallController.list 见 ybc-iot-admin/.../MallController.java:120;复用 StoreService.enabledStores()。".repeat(4) } });
...
await client.callTool({ name: "loop_record", arguments: { artifact: "plan", mode: "overwrite",
  content: "## 方案\n1. 扩展 controller 接口\n2. 新增 DTO 与 service\n## 验证\nmvn -q -pl ybc-iot-admin -am compile" } });
...
await client.callTool({ name: "loop_record", arguments: { artifact: "progress", mode: "overwrite",
  content: "## 进度\n- [x] 扩展接口,兼容原入参\n- [x] 新增按品牌方列门店的 DTO/service\n- [x] 复用 StoreService.enabledStores()" } });
...
ok("最终阶段=DONE 且 profile=dev_warren_agent", state.phase === "DONE" && state.profile === "dev_warren_agent");
